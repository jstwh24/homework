...
-- 兼容 PyMySQL：将认证方式改为 mysql_native_password（防止 caching_sha2_password 报错）
ALTER USER 'game_user'@'%' IDENTIFIED WITH mysql_native_password BY 'game_pass_2024';

-- ----------------------------------------------------------
-- 1. user_account 账号表
-- 存储用户注册信息，密码使用bcrypt哈希加密
-- ----------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS game_save (
    ...
    UNIQUE KEY uk_uid_slot (uid, save_slot),  -- 每个账号每个档位只有一条记录
    INDEX idx_uid (uid)
